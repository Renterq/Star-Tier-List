...
class TesterActionView(discord.ui.View):
    ...
    async def kapat_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not can_manage_kit(interaction.user, self.kit_adi):             custom_id="control_panel_select"
        )
...
async def emojileri_otomatik_bul(interaction: discord.Interaction):
    ...
    if "emojiler" not in AYARLAR: _rol", description="TR: Ban loglarında etiketlenecek rolü ayarlar. / EN: Sets ping role for bans.")
@app_commands.default_permissions(administrator=True)
@sadece_sahip()
...
async def hile_islem_merkezi(interaction, kullanici, nick, sebep, sure_ay, kanit):
    ...
    islem_id = f"HL{random.randint(1000000000, 9999999999)}"Karlı işmiş la bu
...
async def tier_kaldir_merkezi(interaction, nick, kit):
    ...
    kullanici = interaction.guild.get_member(discord_id)=t, value=t) for t in TIERLAR]
)
